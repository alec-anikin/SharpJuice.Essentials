...
namespace SharpJuice.Essentials
{
    public static class ExceptionExtension
    {
        ...
        public static void BindThrow<T>(this Maybe<T> maybe, Exception ex)
        {
            if (maybe.Any())
                throw ex;
        }

        public static void BindThrow<T>(this Maybe<T> maybe, Func<Exception> exceptionFactory)
        {
            if (maybe.Any())
                throw exceptionFactory();
        }

        public static async Task BindThrow<T>(this Task<Maybe<T>> maybeTask, Exception ex)
        {
            if ((await maybeTask).Any())
                throw ex;
        }

        public static async Task BindThrow<T>(this Task<Maybe<T>> maybeTask, Func<Exception> exceptionFactory)
        {
            if ((await maybeTask).Any())
                throw exceptionFactory();
        }
